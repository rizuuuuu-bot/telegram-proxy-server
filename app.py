...
class TelegramProxyServer:
    def __init__(self, host="0.0.0.0", socks_port=1080, mtproto_port=443, sponsor_channel=None, proxy_secret=None):
        ...
        # Default username and password for SOCKS5 authentication
        self.socks_username = os.getenv("SOCKS_USERNAME", "telegramuser")
        self.socks_password = os.getenv("SOCKS_PASSWORD", "telepass123")
    ...
    def handle_socks5_client(self, client_socket):
        ...
        try:
            ...
            if 0x00 in methods: # No authentication
                client_socket.send(b"\x05\x00")
            elif 0x02 in methods: # Username/Password authentication
                client_socket.send(b"\x05\x02")
                # Receive username/password
                auth_data = client_socket.recv(257)
                if not auth_data or auth_data[0] != 0x01: # Auth version
                    logger.warning(f"Invalid SOCKS5 auth request from {client_socket.getpeername()}: {auth_data.hex()}")
                    client_socket.send(b"\x01\xFF") # Auth failed
                    client_socket.close()
                    return
                ...
                if username == self.socks_username and password == self.socks_password:
                    client_socket.send(b"\x01\x00") # Auth success
                    logger.info(f"SOCKS5 authentication successful for user: {username}")
                else:
                    logger.warning(f"SOCKS5 authentication failed for user: {username}")
                    client_socket.send(b"\x01\xFF") # Auth failed
                    client_socket.close()
                    return
            else:
